...
public class EntersektRestletClient{
	  ...
	  private final static String DEBUG_FILE = "GeneratePushNotification";
	...
	public String pushNotify(AuthRq auth) {
		...
		try {
			...
			debug.message("pushNotify Request:  "+ req);
			...
			if (resp.getStatus().isSuccess()) {
				...
				debug.message("pushNotify Response: "+ jsonResponse);
				System.out.println("pushNotify Response: "+ jsonResponse);
				return jsonResponse;

			} else {
				debug.message(resp.getStatus() + "");
			}
	...
	public AuthVerifyResp validatePushNotify(AuthVerifyRq request) {
		...
		try {
			...
			debug.message("validatePushNotify Request:"+ req);
	...
	public String getSignUpCode(SignupRq request) {
		...
		try {
			...
			String req = mapper.writeValueAsString(request);
			debug.message("getSignUpCode Request:"+ req);
			...
			if (resp.getStatus().isSuccess()) {
				...
				if(!jsonResponse.isEmpty()) {
					debug.message("getSignUpCode Response:"+ jsonResponse);
	...
	public String pollSignup(PollSignupRq request) {
		...
		try {
			...
			String req = mapper.writeValueAsString(request);
			debug.message("pollSignup Request:"+ req);
			...
			if (resp.getStatus().isSuccess()) {
				...
				if(!jsonResponse.isEmpty()) {
					debug.message("pollSignup Response:"+ jsonResponse);
	...
	public String verifyOTP(VerifyOTPRq request) {
		...
		try {
			...
			String req = mapper.writeValueAsString(request);
			debug.message("verifyOTP Request:"+ req);
			...
			if (resp.getStatus().isSuccess()) {
				...
				if(!jsonResponse.isEmpty()) {
					debug.message("verifyOTP Response:"+ jsonResponse);
